...
for rep in range(1,numreps+1):
	...
	for x in range(1,threads+1):
		...
		p1[x] = subprocess.Popen("usearch -ublast ~/scripts/tmp2/%subin%d.fna -db ~/data/db/gbbctnt/gbnt.udb -strand both -accel 0.5 -id 0.97  -maxaccepts 25 -maxrejects 256 -maxhits 10 -userout ~/scripts/tmp2/%subin%dout.txt -userfields  query+target+id+ql+qlo+qhi+tlo+thi+tl+alnlen+evalue+bits" %(r2,q,r2,q), shell=True)
