...
for rep in range(1,numreps+1):
	...
	for x in range(1,threads+1):
		...
		p1[x] = subprocess.Popen("usearch -ublast ~/scripts/tmp2/%subin%d.fna -db ~/data/db/genpept/genpept.udb -evalue 1e-10 -accel 0.5 -maxhits %s -maxaccepts 100 -maxrejects 1024 -userout ~/scripts/tmp2/%subin%dout.txt -userfields query+target+id+ql+qlo+qhi+tlo+thi+tl+alnlen+evalue+bits" %(r2,q,maxhits,r2,q), shell=True)
