...
p1= subprocess.Popen("usearch -ublast %s -db %s -strand both -maxhits %s -userout %s -userfields query+target+id+ql+qlo+qhi+tl+tlo+thi+alnlen+evalue+bits -id %s -evalue %s" %(inputname,db,maxhits,outputfile,id,evalue), shell=True).wait() #-ka_ungapped_lambda 0.318 -ka_gapped_lambda 0.291 -ka_gapped_k 0.075 -ka_ungapped_k 0.134

outputfile2=outputfile+".taxa"

print"adding lineage.."
subprocess.Popen("python ~/scripts/linfetch.py %s %s %s %s" %(outputfile,outputfile2, stype, offset1), shell=True).wait()


subprocess.Popen("python ~/scripts/lca2.py %s %s %s %s" %(outputfile2,outputfile+".lca",cons,tax_lev), shell=True).wait()
