...
for rep in range(1,numreps+1):
	...
	for x in range(1,threads+1):
		...
		p1[x] = subprocess.Popen("usearch -ublast ~/scripts/tmp2/ubin%d.fna -db ~/data/db/genpept/sorted1.udb -evalue 1e-10 -accel 0.5 -maxhits %s -maxaccepts 100 -maxrejects 1024 -threads 1 -userout ~/scripts/tmp2/1ubin%dout.txt -userfields query+target+id+ql+qlo+qhi+tlo+thi+tl+alnlen+evalue+bits" %(q,maxhits,q), shell=True)
	
		p2[x] = subprocess.Popen("usearch -ublast ~/scripts/tmp2/ubin%d.fna -db ~/data/db/genpept/sorted2.udb -evalue 1e-10 -accel 0.5 -maxhits %s -maxaccepts 100 -maxrejects 1024 -threads 1 -userout ~/scripts/tmp2/2ubin%dout.txt -userfields query+target+id+ql+qlo+qhi+tlo+thi+tl+alnlen+evalue+bits" %(q,maxhits,q), shell=True)
	
		p3[x] = subprocess.Popen("usearch -ublast ~/scripts/tmp2/ubin%d.fna -db ~/data/db/genpept/sorted3.udb -evalue 1e-10 -accel 0.5 -maxhits %s -maxaccepts 100 -maxrejects 1024 -threads 1 -userout ~/scripts/tmp2/3ubin%dout.txt -userfields query+target+id+ql+qlo+qhi+tlo+thi+tl+alnlen+evalue+bits" %(q,maxhits,q), shell=True)

		p4[x] = subprocess.Popen("usearch -ublast ~/scripts/tmp2/ubin%d.fna -db ~/data/db/genpept/sorted4.udb -evalue 1e-10 -accel 0.5 -maxhits %s -maxaccepts 100 -maxrejects 1024 -threads 1 -userout ~/scripts/tmp2/4ubin%dout.txt -userfields query+target+id+ql+qlo+qhi+tlo+thi+tl+alnlen+evalue+bits" %(q,maxhits,q), shell=True)

		p5[x] = subprocess.Popen("usearch -ublast ~/scripts/tmp2/ubin%d.fna -db ~/data/db/genpept/sorted5.udb -evalue 1e-10 -accel 0.5 -maxhits %s -maxaccepts 100 -maxrejects 1024 -threads 1 -userout ~/scripts/tmp2/5ubin%dout.txt -userfields query+target+id+ql+qlo+qhi+tlo+thi+tl+alnlen+evalue+bits" %(q,maxhits,q), shell=True)
	...
	for x2 in range(1,threads+1):		
		...
		if meta =="m":
			subprocess.Popen("cat ~/scripts/tmp2/%s1ubin%dout.txt ~/scripts/tmp2/%s2ubin%dout.txt ~/scripts/tmp2/%s3ubin%dout.txt ~/scripts/tmp2/%s4ubin%dout.txt ~/scripts/tmp2/%s5ubin%dout.txt >> ~/scripts/tmp2/finaloutput%s.ublast" %(r2,x2,r2,x2,r2,x2,r2,x2,r2,x2), shell=True).wait()
		else:
			subprocess.Popen("cat ~/scripts/tmp2/%s1ubin%dout.txt ~/scripts/tmp2/%s2ubin%dout.txt ~/scripts/tmp2/%s3ubin%dout.txt ~/scripts/tmp2/%s4ubin%dout.txt ~/scripts/tmp2/%s5ubin%dout.txt >> %s" %(r2,x2,r2,x2,r2,x2,r2,x2,r2,x2,outputfile), shell=True).wait()
...
if meta =="m":
	...
	subprocess.Popen("python ~/scripts/taxonfetch.py ~/scripts/tmp2/finaloutput%s.ublast %s p %s" %(outputfile,fetchsize), shell=True).wait()
